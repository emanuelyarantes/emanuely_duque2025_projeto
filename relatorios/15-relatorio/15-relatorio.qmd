...
# Taxas de clique
taxa_a <- cliques_a / enviados_a
taxa_b <- cliques_b / enviados_b

# Diferença
diferenca <- taxa_b - taxa_a
...
# Proporções
p_a <- cliques_a / enviados_a
p_b <- cliques_b / enviados_b

# Diferença observada
dif_obs <- p_b - p_a

# Erro padrão
se <- sqrt((p_a * (1 - p_a)) / enviados_a + (p_b * (1 - p_b)) / enviados_b)

# Intervalo de confiança
z <- 1.96
lim_inf <- dif_obs - z * se
lim_sup <- dif_obs + z * se

# Resultado
lim_inf
lim_sup

# Gráfico da diferença com IC
library(ggplot2)
dados_ic <- data.frame(
  Diferenca = dif_obs,
  IC_inf = lim_inf,
...
ggplot(dados_ic, aes(x = "Diferença", y = Diferenca)) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymin = IC_inf, ymax = IC_sup), width = 0.1) +
  labs(title = "Intervalo de Confiança para Diferença nas Taxas de Clique (B - A)",
       y = "Diferença de Proporções",
       x = "") +
  theme_minimal()
...
Com base nos resultados da análise, observamos que a versão A do e-mail promocional teve uma taxa de cliques de 7,2%, enquanto a versão B apresentou uma taxa de 8,5%. A diferença observada entre as duas versões foi de 1,3 pontos percentuais, favorável à versão B.

Além disso, ao calcular o intervalo de confiança de 95% para a diferença entre as taxas de cliques, obtivemos um intervalo aproximado de [0,6%, 2,0%]. Como esse intervalo não inclui o valor zero, isso indica que a diferença entre as duas versões é estatisticamente significativa. Em outras palavras, há evidências de que a versão B realmente proporciona uma taxa de cliques maior do que a versão A, e essa vantagem não pode ser atribuída ao acaso.

Diante desses resultados, a recomendação é que a empresa adote a versão B do e-mail promocional, com linha de assunto personalizada. Essa versão demonstrou melhor desempenho em atrair cliques dos usuários e, portanto, tem maior potencial de gerar engajamento e conversões, contribuindo positivamente para os objetivos da campanha de marketing.
...
# Q-Q plot para o método tradicional
qqnorm(tradicional, main = "Q-Q Plot - Método Tradicional")
qqline(tradicional)

# Q-Q plot para o método automatizado
qqnorm(automatizado, main = "Q-Q Plot - Método Automatizado")
qqline(automatizado)
...
```{r}
# Calcula as médias
media_tradicional <- mean(tradicional)
media_automatizado <- mean(automatizado)

# Diferença entre as médias (Tradicional - Automatizado)
dif_media <- media_tradicional - media_automatizado
...
t.test(tradicional, automatizado, var.equal = FALSE)

```

4. Interprete os resultados e faça uma recomendação.

Com base nos dados coletados, observou-se que o tempo médio de atendimento com o método tradicional foi de aproximadamente 17,04 minutos, enquanto o novo sistema automatizado apresentou uma média de cerca de 13,32 minutos. A diferença amostral entre os métodos foi de aproximadamente 3,72 minutos, em favor do sistema automatizado.

O intervalo de confiança de 95% para essa diferença, obtido por meio do teste t de Student com variâncias desiguais, indicou que essa diferença é estatisticamente significativa. Como o intervalo não inclui o valor zero, há forte evidência de que o sistema automatizado realmente reduz o tempo médio de resolução de chamadas.

Diante desses resultados, recomenda-se que a empresa adote o novo sistema de triagem automatizada, pois ele oferece uma redução consistente e significativa no tempo de atendimento ao cliente. Essa mudança pode aumentar a eficiência do setor de atendimento e melhorar a experiência dos usuários.
