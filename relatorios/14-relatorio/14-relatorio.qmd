...
μ
 = Qualidade média verdadeira de todas as 8.000 garrafas - parâmetro fixo mas desconhecido.

x¯
: Qualidade média das 120 garrafas avaliadas - estimativa de μ
.
...
μ
 = Ticket médio verdadeiro de todas as 1.200 lojas.

x¯
 = Ticket médio calculado das 80 lojas analisadas.
...
Extraindo uma amostra aleatoria simples com a função slice _simpledo dplyr:
```{r}
#fixa a semente para reprodutibilidade
set.seed(234)

# extrai ASS dos municipios 
amostra_aleatoria_simples <- dados_recentes %>%
  slice_sample(n= 150) %>%
  select(ufn, pop, idhm)
...
Estimador (fórmula/método para estimar σ2
)
Estimativa (valor numérico específico de X¯
)
Parâmetro (p
